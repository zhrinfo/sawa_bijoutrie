...
import { ArrowLeft, Diamond, ShieldCheck, Sparkles, UserPlus } from 'lucide-react'
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(registerUrl, {
        ...
        body: JSON.stringify({ fullName, email, password, roles: ['ROLE_CLIENT'] }),
      })
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#f7f4ee] text-foreground">
      ...
      <section className="relative z-10 mx-auto flex min-h-screen max-w-7xl items-center justify-center px-5 pb-10 pt-28 sm:px-8 lg:px-12">
        <div className="grid w-full max-w-6xl overflow-hidden border border-[#ded6c9] bg-card shadow-[0_30px_90px_rgba(58,45,27,0.12)] lg:grid-cols-[0.9fr_1.1fr]">
          <div className="relative hidden min-h-[760px] overflow-hidden bg-[#203d35] lg:block">
          ...
          <p className="mt-5 text-sm leading-relaxed text-muted-foreground">Créez votre compte pour garder vos favoris et commander vos pièces préférées.</p>
          ...
          <div className="mt-8 flex items-center justify-center gap-2 text-[11px] uppercase tracking-[0.08em] text-muted-foreground"><ShieldCheck className="h-4 w-4 text-gold" /> Une expérience pensée pour vous</div>
          <p className="mt-7 text-center text-sm text-muted-foreground">
            Vous avez déjà un compte ?{' '}
            <Link href="/login" className="text-primary underline underline-offset-4 hover:text-gold">Se connecter</Link>
          </p>
            </div>
          </div>
        </div>
